...
//METODOS DE COMPORTAMIENTO ------------------------------------------------------------------------
		...
		while (estadoDelJuego) {
			...
			if (!turnoActual.estaBloqueado()) {
				...
				if (!turnoActual.estaBloqueado()) {
					...
					RelacionDatoCasillero<Ficha> ultimoMovimiento = new RelacionDatoCasillero<Ficha>(casilleroDestino, casilleroDestino.getDato());
					jugadorActual.setUltimaPosicionMovida(ultimoMovimiento);
	...
	public Casillero<Ficha> mover(Jugador jugador) throws Exception {
		System.out.println("Seleccione la ficha a mover (seleccione ID):");
